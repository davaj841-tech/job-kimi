<template>
  <div
    class="flex min-h-[70dvh] flex-col items-center justify-center px-6 text-center"
  >
    ...
    <template v-else>
      <div
        ...
        :class="
          ok ? 'bg-emerald-50 text-emerald-600' : 'bg-brand-soft text-brand'
        "
      >
        {{ ok ? '✓' : '!' }}
      </div>
      ...
      <div class="flex w-full max-w-xs flex-col gap-2">
        ...
        <RouterLink to="/" class="btn-ghost border border-surface-line"
          >خانه</RouterLink
        >
      </div>
    </template>
  ...
  try {
    ...
    const { data } = await api.post(endpoint, null, {
      params: {
        ...
        ik: route.query.ik,
        idempotency_key: route.query.ik || route.query.idempotency_key,
        pdf_id: route.query.pdf_id,
      },
    })
    ok.value = Boolean(data.success)
    message.value =
      data.message ||
      (ok.value ? 'تراکنش با موفقیت انجام شد.' : 'پرداخت تایید نشد.')
  } catch (e) {
    ok.value = false
