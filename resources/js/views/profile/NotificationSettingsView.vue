<template>
  <div class="mx-auto max-w-xl px-4 py-6">
    ...
    <div v-for="channel in channels" :key="channel.key" class="card-soft mb-4 p-4">
      <h2 class="mb-3 text-sm font-bold">{{ channel.label }}</h2>
      <label v-for="t in types" :key="channel.key + t.key" class="mb-2 flex items-center justify-between text-sm">
        <span>{{ t.label }}</span>
        <input v-model="prefs[channel.key][t.key]" type="checkbox" class="h-4 w-4" />
      </label>
    </div>
    <button class="btn-primary" :disabled="saving" @click="save">{{ saving ? '...' : 'ذخیره' }}</button>
  </div>
</template>
