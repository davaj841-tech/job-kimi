<template>
  <div class="px-4 py-4">
    ...
    <template v-else-if="job">
      <h1 class="mb-1 text-xl font-black text-desk-text">{{ job.title }}</h1>
      <div class="mb-2 flex items-center justify-between gap-2">
        ...
        <button class="text-xs font-bold text-desk-orange" @click="shareOpen = true">اشتراک‌گذاری</button>
      </div>
      <div class="card-soft mb-4 space-y-2 border border-surface-line p-4 text-sm">
        <div>مهلت ثبت‌نام: {{ formatDate(job.registration_deadline) }}</div>
        <div>تاریخ آزمون: {{ formatDate(job.exam_date) }}</div>
        <div>طبقه‌بندی: {{ job.classification_name || job.company_name || '—' }}</div>
        <div v-if="job.seo_tag" class="text-xs text-desk-muted" dir="ltr">SEO: {{ job.seo_tag }}</div>
      </div>
      <div
      ...
      <div v-if="attachments.length" class="mb-4 space-y-2">
        ...
        >
          <span class="font-bold text-desk-orange">{{ file.title || 'دانلود فایل' }}</span>
          <span v-if="file.description" class="mt-0.5 text-xs text-desk-muted">{{ file.description }}</span>
        </a>
      </div>
      <a
      ...
      <section v-if="catalogExams.length" class="mb-5">
        <h2 class="mb-2 text-sm font-black text-desk-text">آزمون‌های مرتبط برای فروش</h2>
        <div class="space-y-2">
          <RouterLink
          ...
          >
            ...
            <span class="text-xs text-desk-muted">{{ exam.is_free ? 'رایگان' : formatPrice(exam.price) }}</span>
          </RouterLink>
        </div>
      </section>

      <section v-if="catalogPdfs.length" class="mb-5">
        <h2 class="mb-2 text-sm font-black text-desk-text">فایل‌های PDF مرتبط</h2>
        <div class="space-y-2">
          <RouterLink
          ...
          >
            ...
            <span class="text-xs text-desk-muted">{{ formatPrice(pdf.price) }}</span>
          </RouterLink>
        </div>
      </section>
...
const shareUrl = computed(() => `${window.location.origin}/jobs/${job.value?.id || ''}`);

const locationLine = computed(() => {
  const j = job.value;
  if (!j) return '';
  const provinces = Array.isArray(j.provinces) && j.provinces.length ? j.provinces.join('، ') : j.province;
