<template>
  <div class="flex min-h-[80dvh] flex-col items-center justify-center px-6 py-12 text-center">
    <div class="mb-6 flex h-28 w-28 items-center justify-center rounded-full bg-brand-soft text-5xl">
      🎓❓
    </div>
    <p class="mb-2 text-5xl font-black text-brand">۴۰۴</p>
    <h1 class="mb-2 text-xl font-black text-ink">صفحه‌ای که دنبالش بودید پیدا نشد!</h1>
    <p class="mb-8 max-w-md text-sm leading-7 text-ink-muted">
      شاید آدرس اشتباه باشد یا صفحه جابه‌جا شده باشد. از گزینه‌های زیر ادامه دهید.
    </p>

    <RouterLink to="/" class="btn-primary mb-8 max-w-xs">برو به صفحه اصلی</RouterLink>

    <form class="mb-10 flex w-full max-w-md gap-2" @submit.prevent="search">
      <input v-model="q" class="input-field flex-1" placeholder="جستجو کنید..." />
      <button type="submit" class="rounded-xl bg-[#0a1c33] px-4 text-sm font-bold text-white">جستجو</button>
    </form>
