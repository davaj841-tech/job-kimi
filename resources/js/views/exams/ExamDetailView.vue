  ...
  <div class="px-4 py-4">
    ...
    <template v-else-if="exam">
      ...
      <div class="card-soft mb-4 grid grid-cols-2 gap-3 p-4 text-sm">
        <div>⏱️ مدت: <b>{{ exam.duration_minutes }} دقیقه</b></div>
      ...
      >
        <p class="font-bold">⚠️ هشدار نمره منفی</p>
        <p class="mt-1">
      ...
      <p v-if="auth.isAuthenticated && !exam.is_eligible && !exam.is_free" class="mb-3 rounded-lg bg-brand-soft p-3 text-sm text-brand">
      ...
      <button
        ...
        :disabled="starting !== false || (auth.isAuthenticated && !(exam.is_eligible || exam.is_free))"
        @click="start(null)"
      >
        {{ starting === 'full' ? 'در حال شروع...' : '🚀 آزمون کامل' }}
      </button>

      <!-- Subjects -->
      <div v-if="exam.subjects?.length" class="mb-4">
        <h2 class="mb-2 text-sm font-bold">📚 دروس این آزمون</h2>
        <div class="space-y-2">
          <div
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <div>
                ...
                <p class="text-[11px] text-ink-muted">{{ s.question_count }} سوال</p>
              </div>
            </div>
            <button
              type="button"
              ...
              :disabled="starting !== false || !s.question_count"
              @click="start(s.slug)"
            >
              {{ starting === s.slug ? '...' : `شروع درس ${s.name}` }}
            </button>
          </div>
        </div>
      </div>
...
const starting = ref(false); // false | 'full' | subjectSlug
const error = ref('');
...
async function start(subjectSlug) {
  ...
  const loginPath = { path: '/login', query: { redirect: `/exams/${exam.value.slug || exam.value.id}` } };
  if (!auth.isAuthenticated) {
    router.push(loginPath);
    return;
  }
  starting.value = subjectSlug || 'full';
  error.value = '';
  try {
    const { data } = await api.post(`/exams/${exam.value.id}/start`, { subject: subjectSlug || null });
