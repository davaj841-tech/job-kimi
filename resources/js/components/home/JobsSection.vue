      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-desk-muted">
            فرصت‌های شغلی ویژه برای آمادگی و اقدام سریع
          </p>
        </div>
        <RouterLink
          to="/jobs"
        ...
        >
          ...
          <div class="p-4 text-right">
            <div class="mb-2 flex flex-wrap items-center justify-end gap-2">
              ...
              <span
                class="rounded-md bg-desk-gray px-2 py-0.5 text-[11px] text-desk-muted"
              >
                {{ job.classification_name || job.company_name || 'عمومی' }}
              </span>
            </div>
            <h3
              class="mb-1 line-clamp-2 text-base font-semibold text-desk-text"
            >
              {{ job.title }}
            </h3>
            <p class="mb-2 text-sm text-desk-muted">
...
function locationOf(job) {
  const provinces =
    Array.isArray(job.provinces) && job.provinces.length
      ? job.provinces.join('، ')
      : job.province
