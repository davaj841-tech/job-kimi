<template>
  <div class="space-y-5">
    <div class="grid gap-5 md:grid-cols-2">
      <div
        class="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-5 dark:bg-slate-800/50"
      >
        <p class="mb-3 text-sm font-bold text-ink dark:text-white">
          نمودار کلی نتیجه
        </p>
        ...
        <div class="mt-3 flex flex-wrap justify-center gap-3 text-[11px]">
          <span class="inline-flex items-center gap-1 text-emerald-600">
            <i class="h-2 w-2 rounded-full bg-emerald-500" /> درست
            {{ fa(correct) }}
          </span>
      ...
      <div class="rounded-2xl bg-slate-50 p-5 dark:bg-slate-800/50">
        <p class="mb-4 text-sm font-bold text-ink dark:text-white">
          تحلیل درس‌ها
        </p>
        ...
        <p v-else class="text-center text-xs text-ink-muted">
          داده درسی موجود نیست.
        </p>
      </div>
    </div>
    ...
    <div class="rounded-2xl bg-slate-50 p-5 dark:bg-slate-800/50">
      ...
      <div v-if="subjects.length" class="overflow-x-auto">
        <div
          class="flex min-w-full items-end justify-start gap-3"
        ...
        >
          ...
          >
            ...
            <div
              class="flex h-40 w-full items-end justify-center rounded-md bg-slate-200/70 px-2 dark:bg-slate-700/60"
            >
              <div
                class="w-8 rounded-t-md transition-all duration-700"
                :class="barClass(row.percentage)"
                :style="{
                  height: `${Math.min(100, Math.max(8, row.percentage || 0))}%`,
                }"
              />
            </div>
            <p
              class="mt-2 w-full text-center text-[11px] font-bold leading-5 text-ink dark:text-slate-200"
            >
              {{ row.emoji || '📘' }}
              <br />
