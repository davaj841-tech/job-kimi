<template>
  <div
    class="-mx-1 flex gap-3 overflow-x-auto px-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
  >
    <button
      type="button"
    ...
    >
      <span
        class="flex h-12 w-12 items-center justify-center rounded-full bg-desk-dark text-white"
      >
    ...
    <button
      ...
      class="flex w-[88px] shrink-0 flex-col items-center gap-2 rounded-2xl border px-2 py-3 text-center transition"
      :class="
        Number(modelValue) === Number(item.id)
          ? 'border-desk-orange bg-orange-50 shadow-sm'
          : 'border-surface-line bg-white hover:border-desk-orange/40'
      "
    ...
    >
      ...
      <span class="line-clamp-2 text-[11px] font-bold text-desk-text">{{
        item.name
      }}</span>
    </button>
  </div>
