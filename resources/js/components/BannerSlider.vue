<template>
  <div v-if="items.length" class="relative overflow-hidden rounded-2xl" @touchstart="onTouchStart" @touchend="onTouchEnd">
    <div class="flex transition-transform duration-500" :style="{ transform: `translateX(${index * 100}%)` }">
      ...
      >
        <img v-if="b.image" :src="b.image" :alt="b.title" class="h-40 w-full object-cover md:h-56" />
        <div v-else class="flex h-40 items-center justify-center bg-[#0a1c33] text-white md:h-56">
...
onMounted(async () => {
  if (!Array.isArray(props.banners)) {
    try {
      const { data } = await api.get('/banners', { params: { position: props.position } });
  ...
  timer = setInterval(() => {
    if (items.value.length > 1) index.value = (index.value + 1) % items.value.length;
  }, 5000);
});
