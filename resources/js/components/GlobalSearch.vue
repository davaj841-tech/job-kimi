<template>
  <div class="relative w-full max-w-xl" ref="root">
    <input
      ...
      :class="dark ? 'border-white/20 bg-white/10 text-white placeholder:text-white/50 focus:border-desk-orange' : 'border-surface-line bg-white text-ink focus:border-brand'"
    ...
    >
      <div v-if="loading" class="p-4 text-center text-xs text-slate-400">در حال جستجو...</div>
      <template v-else-if="q.length >= 2">
        <section v-for="group in groups" :key="group.key" class="border-b border-slate-50 p-2">
          ...
          <button
            ...
            :class="flatIndex(group.key, idx) === activeIndex ? 'bg-orange-50' : 'hover:bg-slate-50'"
            @mouseenter="activeIndex = flatIndex(group.key, idx)"
            @click="go(item)"
          >
            <span v-html="highlight(item.title)"></span>
            <span v-if="item.company_name" class="block text-[11px] text-slate-400">{{ item.company_name }}</span>
          </button>
          <p v-if="!group.items.length" class="px-2 py-1 text-xs text-slate-400">موردی نیست</p>
        </section>
        <RouterLink
      ...
      <div v-else class="p-3">
        <p v-if="recent.length" class="mb-2 text-xs font-bold text-slate-500">جستجوهای اخیر</p>
        <button v-for="r in recent" :key="r" type="button" class="mb-1 block text-sm text-slate-700" @click="q = r; search()">{{ r }}</button>
        <p class="mb-2 mt-3 text-xs font-bold text-slate-500">محبوب</p>
        <button v-for="p in popular" :key="p" type="button" class="mb-1 ml-2 inline-block rounded-full bg-slate-100 px-2 py-1 text-xs" @click="q = p; search()">{{ p }}</button>
      </div>
    </div>
  </div>
</template>
...
const groups = computed(() => [
  { key: 'exams', label: 'آزمون‌ها', items: (results.value.exams || []).map((i) => ({ ...i, type: 'exam' })) },
  { key: 'jobs', label: 'آگهی‌ها', items: (results.value.job_posts || []).map((i) => ({ ...i, type: 'job' })) },
  { key: 'pdfs', label: 'فایل‌ها', items: (results.value.pdfs || []).map((i) => ({ ...i, type: 'pdf' })) },
  { key: 'blog', label: 'بلاگ', items: (results.value.blog_posts || []).map((i) => ({ ...i, type: 'blog' })) },
]);
...
onMounted(() => {
  try { recent.value = JSON.parse(localStorage.getItem('recent_searches') || '[]'); } catch { recent.value = []; }
...
async function search() {
  loading.value = true;
  try {
    const { data } = await api.get('/search/suggestions', { params: { q: q.value.trim() } });
...
function highlight(text) {
  ...
  return escapeHtml(String(text)).replace(new RegExp(`(${term})`, 'gi'), '<mark class="bg-yellow-100 text-inherit rounded px-0.5">$1</mark>');
}
