<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" >
    <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
      ...
      <div class="mb-4 rounded-xl border border-orange-100 bg-orange-50 p-3 text-sm leading-6 text-slate-700">
        <p class="mb-2 font-bold text-orange-700">نمونه فایل XLSX</p>
        <p class="mb-3 text-xs text-slate-600">
          ستون‌ها: title, seo_tag, classification, description, provinces, city, registration_deadline, exam_date, registration_link, is_featured
        </p>
        <button
          type="button"
        ...
        <p v-if="sampleError" class="mt-2 text-xs text-red-500">{{ sampleError }}</p>
      </div>

      <FileUploader v-model="file" accept=".xlsx,.xls,.csv" label="آپلود Excel" hint="ترجیحاً xlsx — حداکثر ۲۰MB" />
      <div v-if="result" class="mt-3 rounded-xl bg-slate-50 p-3 text-sm">
        <p>ایجاد: {{ result.created ?? 0 }}</p>
        <p>رد شده: {{ result.skipped ?? 0 }}</p>
        <ul v-if="result.errors?.length" class="mt-2 max-h-28 overflow-y-auto text-xs text-red-600">
