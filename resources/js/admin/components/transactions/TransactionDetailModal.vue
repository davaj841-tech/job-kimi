  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" >
    <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      ...
      <div v-if="tx" class="space-y-1 text-sm">
        <div v-for="r in rows" :key="r.label" class="flex justify-between gap-4 border-b border-slate-100 py-2">
          <span class="text-slate-500">{{ r.label }}</span>
          <span class="text-left font-medium text-slate-800" dir="auto">{{ r.value }}</span>
        </div>
      </div>
      <div class="mt-5 flex flex-wrap justify-end gap-2">
        ...
        <button type="button" class="btn-muted" @click="$emit('close')">بستن</button>
      </div>
      <p v-if="regenMsg" class="mt-2 text-center text-xs text-slate-600">{{ regenMsg }}</p>
    </div>
  </div>
</template>
...
const rows = computed(() => {
  ...
  return [
    ...
    { label: 'تخفیف', value: tx.discount_amount ? `${fa(tx.discount_amount)} ریال` : '—' },
...
async function regenerate() {
  ...
  regenMsg.value = '';
  try {
    const { data } = await adminApi.post(`/admin/transactions/${props.tx.id}/regenerate-invoice`);
    if (props.tx) {
      props.tx.invoice_number = data.data?.invoice_number || props.tx.invoice_number;
