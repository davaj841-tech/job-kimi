<template>
  <div class="space-y-4">
    <div v-for="field in fields" :key="field.key" class="space-y-1.5">
      ...
      <label v-else-if="field.type === 'toggle'" class="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5 text-sm">
        <span>{{ field.toggleLabel || 'فعال' }}</span>
        <StatusToggle :model-value="isTrue(modelValue[field.key])" @update:model-value="set(field.key, $event ? 'true' : 'false')" />
      </label>
      ...
      <ImageUpload
        ...
        :label="field.label"
        @file="(f) => $emit('upload', { key: field.key, type: field.uploadType || 'logo', file: f })"
      />
    </div>
  </div>
</template>
...
<style scoped>
.label { @apply block text-xs font-bold text-slate-500; }
