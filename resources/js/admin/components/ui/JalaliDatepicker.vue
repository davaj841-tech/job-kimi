<template>
  <div class="grid grid-cols-3 gap-2">
    <select
      class="field"
      :value="parts.jy || ''"
      @change="setPart('jy', $event.target.value)"
    >
    ...
    <select
      class="field"
      :value="parts.jm || ''"
      @change="setPart('jm', $event.target.value)"
    >
    ...
    <select
      class="field"
      :value="parts.jd || ''"
      @change="setPart('jd', $event.target.value)"
    >
...
<style scoped>
.field {
  @apply h-10 w-full rounded-xl border border-slate-200 px-2 text-sm outline-none focus:border-orange-400;
}
</style>
