<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      ...
      <!-- Add / edit form -->
      <form class="mb-5 grid grid-cols-[64px_1fr_1fr_auto] items-end gap-2" @submit.prevent="submit">
        <div>
          <label class="label">آیکون</label>
          <input v-model="form.icon" class="field text-center" placeholder="📘" maxlength="4" />
        </div>
        <div>
          <label class="label">نام درس *</label>
          <input v-model="form.name" class="field" required placeholder="مثلاً ریاضی" />
        </div>
        <div>
          <label class="label">اسلاگ (اختیاری)</label>
          <input v-model="form.slug" class="field text-left" dir="ltr" placeholder="math" />
        </div>
        <button type="submit" class="h-10 rounded-xl bg-orange-500 px-4 text-sm font-bold text-white disabled:opacity-50" :disabled="saving">
          {{ editingId ? 'ذخیره' : 'افزودن' }}
        </button>
      </form>
      <div v-if="editingId" class="-mt-3 mb-4">
        <button type="button" class="text-xs font-bold text-slate-500 underline" @click="resetForm">انصراف از ویرایش</button>
      </div>

      <p v-if="error" class="mb-3 text-sm text-red-500">{{ error }}</p>

      <div v-if="subjectsStore.loading" class="py-8 text-center text-sm text-slate-500">در حال بارگذاری...</div>
      <div v-else class="space-y-2">
        ...
        >
          ...
          <div class="flex gap-1">
            ...
            <button type="button" class="act" @click="startEdit(s)">ویرایش</button>
            <button type="button" class="act text-red-600" @click="remove(s)">حذف</button>
          </div>
        </div>
        <p v-if="!subjectsStore.subjects.length" class="py-6 text-center text-sm text-slate-400">درسی ثبت نشده است</p>
      </div>
    </div>
  </div>
</template>
...
async function remove(s) {
  ...
  try {
    ...
    toast.error(e.response?.data?.message || 'حذف ناموفق بود — احتمالاً در سوالات استفاده شده است.');
  }
}
