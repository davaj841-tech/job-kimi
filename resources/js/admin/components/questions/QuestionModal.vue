    ...
    >
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-bold">
          {{ question?.id ? 'ویرایش سوال' : 'سوال جدید' }}
        </h3>
      ...
      <form class="space-y-4" @submit.prevent="submit">
        ...
        <div class="flex justify-end gap-2">
          <button
            type="button"
            class="rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-bold"
            @click="$emit('close')"
          >
            انصراف
          </button>
          <button
            type="submit"
            class="rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white"
            :disabled="saving"
          >
            {{ saving ? '...' : 'ذخیره' }}
          </button>
