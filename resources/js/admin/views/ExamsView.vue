  ...
  <AdminLayout>
    <div class="space-y-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          class="rounded-xl bg-[#0f2744] px-4 py-2.5 text-sm font-bold text-white"
          @click="openCreate"
        >
          آزمون جدید
        </button>
      ...
      >
        ...
        <template #cell-title="{ row }">
          <button
            class="text-right font-medium text-slate-800 hover:text-orange-600"
            @click="openEdit(row)"
          >
            {{ row.title }}
          </button>
        ...
        <template #cell-question_count="{ row }">{{
          fa(row.question_count)
        }}</template>
        <template #cell-duration_minutes="{ row }"
          >{{ fa(row.duration_minutes) }} دقیقه</template
        >
        ...
        <template #actions="{ row }">
          <div class="flex flex-wrap justify-end gap-1">
            <RouterLink
              class="act"
              :to="{ name: 'admin-exam-take', params: { id: row.id } }"
              >آزمون‌گیری</RouterLink
            >
            <button class="act" @click="openEdit(row)">ویرایش</button>
            <RouterLink
              class="act"
              :to="{ path: '/admin/questions', query: { exam_id: row.id } }"
              >سوالات</RouterLink
            >
            <button class="act" @click="openStats(row)">آمار</button>
            <button class="act text-red-600" @click="askArchive(row)">
              بایگانی
            </button>
          </div>
        </template>
      ...
      <div
        v-if="store.meta.last_page"
        class="flex items-center justify-between rounded-xl bg-white px-4 py-3 text-sm shadow-sm"
      >
        <p class="text-slate-500">
          نمایش {{ fa(store.meta.from || 0) }} تا
          {{ fa(store.meta.to || 0) }} از {{ fa(store.meta.total || 0) }}
        </p>
...
function statusLabel(s) {
  return (
    { published: 'منتشر شده', draft: 'پیش‌نویس', archived: 'بایگانی' }[s] || s
  )
}
...
onMounted(async () => {
  await Promise.all([store.fetchClassifications(), store.fetchExams(1)])
})
</script>
