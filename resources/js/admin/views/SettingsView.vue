    ...
    <div class="space-y-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <span v-if="store.dirty" class="text-xs text-orange-600">تغییرات ذخیره‌نشده</span>
      ...
      <div class="rounded-xl bg-white p-5 shadow-sm">
        <div v-if="store.loading" class="py-10 text-center text-sm text-slate-400">در حال بارگذاری...</div>
        <SettingsForm
          v-else
          v-model="form"
...
const fieldMap = {
  general: [
    ...
    { key: 'site_favicon', label: 'فاوآیکون', type: 'image', uploadType: 'favicon' },
  ...
  mail: [
    ...
    { key: 'smtp_from_address', label: 'From Address', type: 'email', ltr: true },
    { key: 'smtp_from_name', label: 'From Name', type: 'text' },
  ],
  theme: [
    { key: 'primary_color', label: 'رنگ اصلی', type: 'color' },
    { key: 'secondary_color', label: 'رنگ ثانویه', type: 'color' },
    { key: 'logo_light', label: 'لوگوی روشن', type: 'image', uploadType: 'logo_light' },
    { key: 'logo_dark', label: 'لوگوی تاریک', type: 'image', uploadType: 'logo_dark' },
  ],
  seo: [
    { key: 'meta_title', label: 'Meta Title پیش‌فرض', type: 'text' },
    { key: 'meta_description', label: 'Meta Description', type: 'textarea' },
    { key: 'meta_keywords', label: 'کلمات کلیدی', type: 'text' },
    { key: 'google_analytics_id', label: 'Google Analytics ID', type: 'text', ltr: true },
    { key: 'google_tag_manager', label: 'Google Tag Manager', type: 'text', ltr: true },
  ],
  payment: [
    {
      key: 'payment_gateway',
    ...
    { key: 'zarinpal_merchant_id', label: 'شناسه پذیرنده زرین‌پال', type: 'text', ltr: true },
    { key: 'zarinpal_sandbox', label: 'Sandbox زرین‌پال', type: 'toggle' },
    { key: 'nextpay_api_key', label: 'API Key نکست‌پی', type: 'text', ltr: true },
  ...
  sms: [
    ...
    { key: 'sms_subscription_reminder_template', label: 'قالب یادآوری اشتراک', type: 'textarea' },
  ],
  ai: [
    {
      key: 'ai_provider',
  ...
  security: [
    { key: 'turnstile_site_key', label: 'Cloudflare Turnstile Site Key', type: 'text', ltr: true },
    { key: 'turnstile_secret_key', label: 'Secret Key', type: 'text', ltr: true },
    { key: 'turnstile_enabled', label: 'فعال بودن Turnstile', type: 'toggle' },
    { key: 'captcha_enabled', label: 'فعال بودن Captcha (سازگاری)', type: 'toggle' },
  ],
  social: [
...
onBeforeRouteLeave((_to, _from, next) => {
  if (store.dirty && !window.confirm('تغییرات ذخیره‌نشده دارید. خارج می‌شوید؟')) {
    next(false);
  } else {
    next();
  }
...
function switchSection(key) {
  if (store.dirty && !window.confirm('تغییرات این بخش ذخیره نشده. ادامه؟')) return;
  active.value = key;
  store.dirty = false;
  loadForm();
}
