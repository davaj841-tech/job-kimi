  ...
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-5">
      <select v-model="filters.user_id" class="field">
        ...
        <option v-for="u in report.operators || []" :key="u.id" :value="u.id">
    ...
    <p v-if="dateRangeError" class="text-xs text-red-600">
      {{ dateRangeError }}
    </p>
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <div class="rounded-2xl bg-white p-4 shadow-sm">
        ...
        <ul class="max-h-64 space-y-2 overflow-y-auto text-sm">
          ...
          <li v-if="!(report.by_action || []).length" class="text-slate-400">
            داده‌ای نیست
          </li>
        </ul>
      </div>
      <div class="rounded-2xl bg-white p-4 shadow-sm">
        ...
        <ul class="max-h-64 space-y-2 overflow-y-auto text-sm">
          ...
          >
            <span
              >{{ row.name }}
              <span class="text-xs text-slate-400">{{ row.role }}</span></span
            >
            <span class="font-bold">{{ fa(row.total) }}</span>
          </li>
