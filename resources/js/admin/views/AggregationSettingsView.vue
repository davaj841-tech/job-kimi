    ...
    <div class="space-y-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button class="btn-muted" :disabled="busy" @click="runDispatch(true)">
            Dry-run
          </button>
          <button
            class="btn-orange"
      ...
      <template v-else>
        <div class="rounded-xl bg-white p-5 shadow-sm">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              <label class="mb-1 block text-xs text-slate-500"
                >تأخیر بین منابع (ثانیه)</label
              >
              <input
                v-model.number="form.dispatch_delay_seconds"
                type="number"
                min="0"
                max="300"
                class="field"
            ...
            <div>
              <label class="mb-1 block text-xs text-slate-500"
                >تلاش مجدد صف (tries)</label
              >
              <input
                v-model.number="form.retry_tries"
                type="number"
                min="1"
                max="10"
                class="field"
          ...
          <p v-if="formOk" class="mt-3 text-sm text-emerald-700">
            {{ formOk }}
          </p>
        ...
        <div class="rounded-xl bg-white p-5 shadow-sm">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <span class="text-xs text-slate-400"
              >حداکثر {{ meta.max_times || 24 }} زمان</span
            >
          </div>

          <div
            class="mb-4 grid grid-cols-1 gap-2 rounded-xl border border-dashed p-3 md:grid-cols-4"
          >
            <input
              v-model="newTime.time"
              class="field"
              dir="ltr"
              placeholder="HH:MM مثلاً 08:00"
            />
            <input
              v-model="newTime.label"
              class="field"
              placeholder="برچسب اختیاری"
            />
            <label class="flex items-center gap-2 text-sm"
              ><input v-model="newTime.enabled" type="checkbox" /> فعال</label
            >
            <button class="btn-dark" :disabled="busy" @click="addTime">
              افزودن زمان
            </button>
          </div>

          <div
            v-if="!form.times.length"
            class="rounded-xl bg-slate-50 p-4 text-sm text-slate-500"
          >
            هنوز زمانی تعریف نشده است. بدون زمان فعال، تجمیع خودکار اجرا
            نمی‌شود.
          </div>

          <div
            v-for="row in form.times"
            :key="row.id"
            class="mb-2 grid grid-cols-1 items-center gap-2 rounded-xl border p-3 md:grid-cols-5"
          >
            ...
            />
            <label class="flex items-center gap-2 text-sm"
              ><input v-model="row.enabled" type="checkbox" /> فعال</label
            >
            <div class="flex justify-end gap-2">
              <button class="act" :disabled="busy" @click="saveTime(row)">
                ذخیره
              </button>
              <button
                class="act text-red-600"
                :disabled="busy"
                @click="removeTime(row)"
              >
                حذف
              </button>
            </div>
        ...
        <div
          class="rounded-xl bg-slate-50 p-4 text-xs leading-6 text-slate-600"
        >
          <p class="font-semibold text-slate-700">یادداشت سرور</p>
          <p>{{ meta.timezone_note }}</p>
          <p dir="ltr" class="font-mono">
            {{ meta.server_scheduler_requirement }}
          </p>
          <p>
            منابع می‌توانند «زمان‌بندی سراسری» یا «زمان‌بندی سفارشی» داشته باشند
            (در صفحه منابع تجمیع).
          </p>
        </div>
