...
function toFormData(payload: Record<string, unknown> | null | undefined) {
  ...
  const titles = Array.isArray(data.attachment_titles)
    ? data.attachment_titles
    : []
  const descriptions = Array.isArray(data.attachment_descriptions)
    ? data.attachment_descriptions
    : []
  ...
  if (Array.isArray(data.remove_attachment_ids)) {
    form.append(
      'remove_attachment_ids',
      JSON.stringify(data.remove_attachment_ids)
    )
  }
