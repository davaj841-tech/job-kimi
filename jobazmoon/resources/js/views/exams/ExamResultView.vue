  ...
  <div class="px-4 py-6">
    ...
    <template v-else-if="result">
      <div class="card-soft mb-4 p-5 text-center">
        ...
        <p class="text-4xl font-black text-brand">{{ Math.round(result.percentage || 0) }}%</p>
        <p class="mt-2 text-sm">نمره: {{ result.score }}</p>
        <p v-if="analysis?.rank" class="mt-1 text-xs text-ink-muted">رتبه: {{ analysis.rank }}</p>
        <p
      ...
      <div class="mb-4 grid grid-cols-2 gap-2 text-sm">
        <div class="card-soft p-3 text-center">صحیح: {{ result.total_correct }}</div>
        <div class="card-soft p-3 text-center">غلط: {{ result.total_wrong }}</div>
      </div>
          ...
          <div v-for="row in subjects" :key="row.subject" class="text-xs">
            ...
            <div class="h-1.5 overflow-hidden rounded bg-surface-page">
              <div
                ...
                :style="{ width: `${row.total ? Math.round((row.correct / row.total) * 100) : 0}%` }"
              />
            </div>
          </div>
        </div>
      ...
      <RouterLink to="/exams" class="btn-primary mt-2">بازگشت به آزمون‌ها</RouterLink>
