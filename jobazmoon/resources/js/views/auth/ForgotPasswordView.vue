<template>
  <div class="flex min-h-dvh items-center justify-center bg-surface-page px-4 py-6" dir="rtl">
    <div class="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-xl">
      ...
      <div class="px-5 py-5">
        <!-- Step 1: identifier -->
        <form v-if="step === 'identify'" class="space-y-3" @submit.prevent="submitIdentify">
          <label class="mb-1 block text-xs font-bold text-ink-soft">📧 ایمیل یا 📱 شماره موبایل</label>
          <input
            v-model="identifier"
        ...
        <form v-else-if="step === 'otp'" class="space-y-3" @submit.prevent="submitOtp">
          <p class="text-xs text-ink-muted">📩 کد ارسال‌شده به {{ identifier }} را وارد کنید و رمز جدید تعیین کنید.</p>
          <div>
            <label class="mb-1 block text-xs font-bold text-ink-soft">🔢 کد تایید</label>
            <input
              v-model="code"
          ...
          <div>
            <label class="mb-1 block text-xs font-bold text-ink-soft">🔒 رمز عبور جدید</label>
            <input v-model="password" type="password" class="input-field" required autocomplete="new-password" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-bold text-ink-soft">🔁 تکرار رمز عبور</label>
            <input
          ...
          <button type="button" class="btn-ghost w-full" @click="step = 'identify'">بازگشت</button>
        </form>

        <!-- Email success -->
        <div v-else-if="step === 'email'" class="space-y-3">
          <p class="rounded-xl bg-green-50 p-4 text-center text-sm text-green-800">
            ✅ در صورت وجود حساب، لینک بازنشانی به ایمیل شما ارسال شد. صندوق ورودی و اسپم را بررسی کنید.
          </p>
          <RouterLink to="/login" class="btn-primary block text-center">بازگشت به ورود</RouterLink>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          <RouterLink to="/login" class="btn-primary block text-center">ورود</RouterLink>
        </div>

        <p v-if="error" class="mt-3 text-center text-xs font-medium text-brand">{{ error }}</p>

        <RouterLink v-if="step === 'identify'" to="/login" class="mt-5 block text-center text-xs text-ink-muted underline">
          بازگشت به ورود
        </RouterLink>
      </div>
    </div>
...
async function submitIdentify() {
  ...
  try {
    ...
    const channel = data.data?.channel || ( /^09\d{9}$/.test(id) ? 'mobile' : 'email');
    if (channel === 'mobile') {
      step.value = 'otp';
    } else {
      step.value = 'email';
