<template>
  <div>
    <div
      ...
      :class="dragging ? 'border-orange-400 bg-orange-50' : 'border-slate-200 bg-slate-50 hover:border-orange-300'"
      @click="pick"
    ...
    >
      ...
      <p v-if="fileName" class="mt-3 text-xs font-medium text-orange-600">{{ fileName }}</p>
      <div v-if="progress > 0 && progress < 100" class="mt-3 h-1.5 w-40 overflow-hidden rounded bg-slate-200">
        <div class="h-full bg-orange-500 transition-all" :style="{ width: `${progress}%` }" />
      </div>
      <img v-if="previewUrl" :src="previewUrl" alt="" class="mt-3 h-24 w-24 rounded-lg object-cover" />
    </div>
...
const fileName = computed(() => (props.modelValue instanceof File ? props.modelValue.name : ''));
