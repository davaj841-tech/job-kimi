<template>
  ...
  >
    <p class="text-slate-500">
      نمایش {{ fa(meta.from || 0) }} تا {{ fa(meta.to || 0) }} از {{ fa(meta.total || 0) }}
    </p>
    <div class="flex gap-1">
      <button :disabled="(meta.current_page || 1) <= 1" @click="$emit('page', (meta.current_page || 1) - 1)">قبلی</button>
      <button
        v-for="p in pages"
        :key="p"
        class="min-w-8 rounded-lg px-2 py-1 text-xs font-bold"
        :class="p === meta.current_page ? 'bg-orange-500 text-white' : 'bg-slate-100'"
        @click="$emit('page', p)"
      >
        {{ fa(p) }}
      </button>
