  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" >
    <div class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-bold">{{ post?.id ? 'ویرایش آگهی' : 'آگهی جدید' }}</h3>
      ...
      <form class="space-y-3" @submit.prevent="submit">
        <div>
          ...
          <input v-model="form.title" required class="field" placeholder="عنوان آگهی" />
        </div>
        ...
        <div>
          ...
          <input
            ...
            @blur="normalizeSeo"
          />
          <p class="mt-1 text-[11px] text-slate-400">برای جستجوی گوگل؛ حروف، عدد و _ مجاز است.</p>
        </div>
        ...
        <div>
          ...
          <p class="mt-1 text-[11px] text-slate-400">مدیریت مادر/فرزند از دکمه «طبقه‌بندی‌ها» کنار آگهی جدید</p>
        </div>
        ...
        <div>
          ...
          <div class="max-h-36 overflow-y-auto rounded-xl border border-slate-200 p-2">
            <label
              v-for="p in allProvinces"
              :key="p"
              class="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 text-sm hover:bg-slate-50"
            >
              <input v-model="form.provinces" type="checkbox" :value="p" class="rounded border-slate-300 text-orange-500" />
              {{ p }}
            </label>
          </div>
        </div>
        ...
        <input v-model="form.registration_link" class="field" dir="ltr" placeholder="لینک ثبت‌نام" />
        <input v-model="form.source_url" class="field" dir="ltr" placeholder="منبع" />

        <div class="rounded-xl border border-slate-200 p-3">
          <div class="mb-2 flex items-center justify-between">
            <label class="label mb-0">فایل‌های مربوطه (چندتایی — اختیاری)</label>
            <button type="button" class="text-xs font-bold text-orange-600" @click="addFileRow">+ فایل</button>
          </div>
          ...
          <div v-if="existingAttachments.length" class="mb-3 space-y-2">
            ...
            >
              <div>
                <a :href="att.url" target="_blank" class="font-bold text-orange-600">{{ att.title || 'فایل' }}</a>
                <p v-if="att.description" class="text-xs text-slate-500">{{ att.description }}</p>
              </div>
              <button type="button" class="text-xs font-bold text-red-600" @click="markRemove(att.id)">حذف</button>
            </div>
          </div>

          <div v-for="(row, idx) in form.newFiles" :key="idx" class="mb-3 space-y-2 rounded-xl border border-dashed border-slate-200 p-3">
            <input type="file" class="block w-full text-xs" accept=".pdf,.doc,.docx,.zip,.rar,.jpg,.jpeg,.png" @change="onFilePick(idx, $event)" />
            <input v-model="row.title" class="field" placeholder="عنوان فایل (مثال: دفترچه راهنما)" />
            <input v-model="row.description" class="field" placeholder="شرح فایل" />
            <button type="button" class="text-xs text-red-600" @click="form.newFiles.splice(idx, 1)">حذف ردیف</button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <label class="flex items-center justify-between rounded-xl border border-slate-200 px-3 text-sm">
            ویژه
...
watch(
  ...
  () => {
    ...
    const extra = (props.provinces || []).filter((p) => !IRAN_PROVINCES.includes(p));
...
function map(p) {
  return {
    ...
    provinces: Array.isArray(p.provinces) ? [...p.provinces] : (p.province ? [p.province] : []),
    city: p.city || '',
...
function submit() {
  ...
  try {
    ...
    if (!form.description || form.description === '<br>' || form.description === '<div><br></div>') {
    ...
    const payload = {
      ...
      attachments: form.newFiles.filter((r) => r.file instanceof File).map((r) => r.file),
      attachment_titles: form.newFiles.filter((r) => r.file instanceof File).map((r) => r.title || ''),
      attachment_descriptions: form.newFiles.filter((r) => r.file instanceof File).map((r) => r.description || ''),
...
.label { @apply mb-1 block text-xs text-slate-500; }
