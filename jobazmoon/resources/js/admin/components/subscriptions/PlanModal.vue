  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" >
    <div class="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-bold">{{ plan?.id ? 'ویرایش پلن' : 'پلن جدید' }}</h3>
        <button type="button" @click="$emit('close')">✕</button>
      </div>

      <form class="space-y-3" @submit.prevent="submit">
        <input v-model="form.name" required class="field" placeholder="نام پلن *" />
        ...
        <input v-model.number="form.price" type="number" min="0" required class="field" placeholder="قیمت (تومان) *" />

        <div>
          <label class="label mb-2 block">ویژگی‌ها</label>
          <div v-for="(f, i) in form.features" :key="i" class="mb-2 flex gap-2">
            <input v-model="form.features[i]" class="field flex-1" placeholder="ویژگی" />
            <button type="button" class="btn-muted px-3" @click="form.features.splice(i, 1)">حذف</button>
          </div>
          <button type="button" class="text-sm font-bold text-orange-600" @click="form.features.push('')">+ افزودن ویژگی</button>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <input v-model="form.badge_color" class="field" dir="ltr" placeholder="رنگ برچسب (#f97316)" />
        </div>
...
<style scoped>
.label { @apply text-xs font-bold text-slate-500; }
