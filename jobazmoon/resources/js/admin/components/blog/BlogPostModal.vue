  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" >
    <div class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-bold">{{ post?.id ? 'ویرایش مقاله' : 'مقاله جدید' }}</h3>
        <button @click="$emit('close')">✕</button>
      </div>

      <form class="space-y-3" @submit.prevent="submit">
        <input v-model="form.title" required class="field" placeholder="عنوان *" @input="onTitle" />
        <input v-model="form.slug" class="field" dir="ltr" placeholder="slug" />
        <textarea v-model="form.excerpt" maxlength="500" rows="2" class="field h-auto" placeholder="خلاصه (حداکثر ۵۰۰)" />
        ...
        <FileUploader
          ...
          :max-size-mb="2"
        />
        <input v-model="form.category" required class="field" placeholder="دسته‌بندی *" />
        <input v-model="form.meta_title" class="field" placeholder="meta_title" />
        <textarea v-model="form.meta_description" rows="2" class="field h-auto" placeholder="meta_description" />
...
watch(
  ...
  () => {
    ...
    clearTimeout(timer);
    timer = setTimeout(() => {
      const { featured_image_file, ...rest } = form;
