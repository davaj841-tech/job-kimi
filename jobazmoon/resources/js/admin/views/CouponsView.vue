  ...
  <AdminLayout>
    <div class="space-y-5">
      ...
      <div class="flex flex-wrap gap-2">
        <input v-model="search" class="field max-w-xs" placeholder="جستجوی کد..." @keyup.enter="load" />
        <select v-model="activeFilter" class="field max-w-[10rem]" @change="load">
      ...
      <DataTable :columns="columns" :rows="rows" :loading="loading" actions>
        ...
        <template #cell-type="{ row }">{{ row.type === 'percentage' ? 'درصدی' : 'مبلغ ثابت' }}</template>
        <template #cell-value="{ row }">
          {{ row.type === 'percentage' ? `${fa(row.value)}٪` : fa(row.value) }}
        </template>
        <template #cell-uses="{ row }">{{ fa(row.used_count) }} / {{ row.max_uses != null ? fa(row.max_uses) : '∞' }}</template>
        <template #cell-applicable_to="{ row }">{{ applicableLabel(row.applicable_to) }}</template>
        <template #cell-is_active="{ row }">{{ row.is_active ? 'فعال' : 'غیرفعال' }}</template>
    ...
    <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form class="max-h-[90vh] w-full max-w-lg space-y-3 overflow-y-auto rounded-2xl bg-white p-5" @submit.prevent="save">
        <h3 class="font-bold">{{ form.id ? 'ویرایش کد تخفیف' : 'کد تخفیف جدید' }}</h3>
        <input v-model="form.code" required class="field uppercase" dir="ltr" placeholder="کد (مثلاً SAVE20)" />
        <select v-model="form.type" class="field">
          <option value="percentage">درصدی</option>
          <option value="fixed">مبلغ ثابت</option>
        </select>
        <input v-model.number="form.value" required type="number" min="0" class="field" placeholder="مقدار" />
        <input v-model.number="form.max_uses" type="number" min="1" class="field" placeholder="حداکثر استفاده (خالی = نامحدود)" />
        <input v-model.number="form.min_purchase" type="number" min="0" class="field" placeholder="حداقل مبلغ خرید" />
