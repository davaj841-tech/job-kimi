    ...
    <div class="space-y-5">
      ...
      <div class="rounded-xl bg-white p-4 shadow-sm">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          <input v-model="store.filters.search" class="field" placeholder="جستجو" @keyup.enter="apply" />
      ...
      <DataTable :columns="columns" :rows="store.products" :loading="store.loading" actions>
        ...
        <template #cell-download_count="{ row }">{{ fa(row.download_count) }}</template>
        <template #cell-is_active="{ row }">
          <StatusToggle :model-value="Boolean(row.is_active)" @update:model-value="(v) => onToggle(row, v)" />
        </template>
        <template #actions="{ row }">
          <div class="flex flex-wrap justify-end gap-1">
            <button class="act" @click="openEdit(row)">ویرایش</button>
            <button class="act" @click="toast.info(`دانلودها: ${fa(row.download_count)}`)">آمار دانلود</button>
