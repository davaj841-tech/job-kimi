<template>
  <AdminLayout>
    ...
    <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form class="w-full max-w-lg space-y-3 rounded-2xl bg-white p-5" @submit.prevent="save">
        ...
        <input v-model.number="form.sort_order" type="number" class="field" placeholder="ترتیب" />
...
const file = ref(null);
const form = reactive({ id: null, title: '', link: '', position: 'home_top', sort_order: 0, is_active: true });
const columns = [
  { key: 'index', label: '#' },
...
function openCreate() {
  Object.assign(form, { id: null, title: '', link: '', position: 'home_top', sort_order: 0, is_active: true });
  file.value = null;
  modal.value = true;
}
function edit(row) {
