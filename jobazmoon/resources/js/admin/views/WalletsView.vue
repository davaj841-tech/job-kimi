  ...
  <AdminLayout>
    <div class="space-y-5">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <StatCard title="مجموع موجودی کاربران" :value="fa(store.stats.total_balance)" icon="₪" color="#0f2744" />
        <StatCard title="تعداد شارژ امروز" :value="fa(store.stats.charges_today)" icon="◎" color="#2563eb" />
        <StatCard title="مبلغ شارژ امروز" :value="fa(store.stats.charge_amount_today)" icon="+" color="#059669" />
      </div>

      <div class="rounded-xl bg-white p-4 shadow-sm">
        <div class="flex flex-wrap gap-2">
          <input v-model="store.filters.search" class="field max-w-md" placeholder="جستجو نام/موبایل" @keyup.enter="apply" />
          <button class="btn-orange" @click="apply">جستجو</button>
        </div>
      </div>

      <DataTable :columns="columns" :rows="store.wallets" :loading="store.loading" actions>
        ...
        <template #cell-total_charged="{ row }">{{ fa(row.total_charged) }}</template>
        <template #cell-total_withdrawn="{ row }">{{ fa(row.total_withdrawn) }}</template>
        <template #cell-last_transaction_at="{ row }">{{ formatDateTime(row.last_transaction_at) }}</template>
        <template #actions="{ row }">
          <div class="flex flex-wrap justify-end gap-1">
            <button class="act" @click="openHistory(row)">تاریخچه</button>
            <button class="act text-emerald-700" @click="openAdjust(row, 'charge')">شارژ</button>
            <button class="act text-red-600" @click="openAdjust(row, 'deduct')">کسر</button>
          </div>
        </template>
      </DataTable>
