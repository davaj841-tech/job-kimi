<template>
  <AdminLayout>
    <div class="space-y-5">
      ...
      <div class="rounded-xl bg-white p-4 shadow-sm">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          <input v-model="store.filters.search" class="field" placeholder="جستجو عنوان" @keyup.enter="apply" />
          <select v-model="store.filters.status" class="field">
            <option value="">همه وضعیت‌ها</option>
            <option value="draft">پیش‌نویس</option>
            <option value="published">منتشر شده</option>
          </select>
          <input v-model="store.filters.category" class="field" placeholder="دسته‌بندی" />
        </div>
      ...
      <DataTable :columns="columns" :rows="store.posts" :loading="store.loading" actions>
        ...
        <template #cell-status="{ row }">
          <span class="rounded-full px-2 py-0.5 text-xs font-bold" :class="row.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'">
        ...
        <template #actions="{ row }">
          <div class="flex flex-wrap justify-end gap-1">
            ...
            <a class="act" :href="`/blog/${row.slug}`" target="_blank" rel="noopener">پیش‌نمایش</a>
    ...
    <BlogPostModal :open="modalOpen" :post="editing" @close="modalOpen = false" @saved="onSaved" />
    <BlogAIGenerateModal ref="aiRef" :open="aiOpen" @close="aiOpen = false" @generate="onGenerate" />
...
function rowNum(i) {
  return ((store.meta.current_page || 1) - 1) * (store.meta.per_page || 20) + i + 1;
}
