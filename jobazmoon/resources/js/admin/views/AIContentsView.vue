  ...
  <AdminLayout>
    <div class="space-y-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-2xl font-bold text-gray-800">مدیریت محتوای هوش مصنوعی</h1>
        <p class="text-sm text-slate-500">
          تولید امروز: {{ fa(store.stats.generated_today) }} / سقف روزانه: {{ fa(store.stats.daily_limit) }}
        </p>
      </div>

      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard title="تولید امروز" :value="fa(store.stats.generated_today)" icon="✦" color="#7c3aed" />
        <StatCard title="سقف روزانه" :value="fa(store.stats.daily_limit)" icon="▣" color="#0f2744" />
        <StatCard title="در انتظار تأیید" :value="fa(store.stats.pending)" icon="◎" color="#eab308" />
        <StatCard title="سوالات ذخیره‌شده" :value="fa(store.stats.by_type?.exam_question || 0)" icon="?" color="#2563eb" />
      </div>
      ...
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :key="t.value"
          class="btn-muted"
          :class="store.filters.type === t.value ? 'ring-2 ring-orange-400' : ''"
      ...
      <div class="rounded-xl bg-white p-4 shadow-sm">
        <select v-model="store.filters.status" class="field max-w-xs" @change="apply">
      ...
      <DataTable :columns="columns" :rows="store.items" :loading="store.loading" actions>
...
function statusLabel(s) {
  return { pending: 'در انتظار تأیید', approved: 'تایید شده', rejected: 'رد شده' }[s] || s;
}
...
function topicOf(row) {
  return row.prompt || row.metadata?.topic || row.metadata?.subject || `#${row.id}`;
}
function metaSummary(row) {
  const m = row.metadata || {};
  if (row.type === 'exam_question') {
    return [m.subject, m.difficulty, m.count ? `${m.count} سوال` : ''].filter(Boolean).join(' / ') || '—';
  }
  return m.source || '—';
}
