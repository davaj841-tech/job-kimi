<template>
  <AdminLayout>
    <div class="mx-auto max-w-3xl">
      ...
      <div v-if="loading" class="rounded-2xl bg-white p-10 text-center text-sm text-slate-500 shadow-sm">در حال بارگذاری نتیجه...</div>
      ...
      <template v-else-if="attempt">
        <div class="mb-4 rounded-2xl bg-white p-6 text-center shadow-sm">
          ...
          <p class="mt-2 text-5xl font-black text-orange-600">{{ fa(Math.round(attempt.percentage || 0)) }}٪</p>
          <p class="mt-2 text-sm text-slate-600">نمره: {{ fa(attempt.score) }} از {{ fa(exam?.total_marks || 0) }}</p>
          <p
            class="mt-3 inline-block rounded-lg px-3 py-1 text-xs font-bold"
            :class="passed ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'"
          >
            {{ passed ? '🎉 قبول' : '📌 مردود' }}
          </p>
        </div>
        ...
        <div class="mb-4 grid grid-cols-3 gap-3 text-center">
          <div class="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <p class="mt-1 text-xl font-black text-emerald-600">{{ fa(attempt.total_correct) }}</p>
          </div>
          <div class="rounded-2xl bg-white p-4 shadow-sm">
            <p class="text-[11px] text-slate-500">غلط</p>
            <p class="mt-1 text-xl font-black text-red-500">{{ fa(attempt.total_wrong) }}</p>
          </div>
          <div class="rounded-2xl bg-white p-4 shadow-sm">
            <p class="text-[11px] text-slate-500">بی‌پاسخ</p>
            <p class="mt-1 text-xl font-black text-slate-500">{{ fa(blankCount) }}</p>
          </div>
        </div>

        <div class="mb-4 rounded-2xl bg-white p-5 text-sm leading-7 text-slate-600 shadow-sm">
          ...
          <template v-if="blankCount"> و <b>{{ fa(blankCount) }}</b> بی‌پاسخ</template>.
          نتیجه ذخیره شد و در آمار و تعداد شرکت‌کنندگان آزمون قابل مشاهده است.
        </div>

        <div v-if="subjects.length" class="mb-4 rounded-2xl bg-white p-5 shadow-sm">
            ...
            <div v-for="row in subjects" :key="row.subject" class="text-xs">
              <div class="mb-1 flex items-center justify-between">
                ...
                <span class="text-slate-500">
                  ✅ {{ fa(row.correct) }} · ❌ {{ fa(row.wrong) }} · ➖ {{ fa(row.blank) }} / {{ fa(row.total) }}
                </span>
              </div>
              <div class="h-1.5 overflow-hidden rounded bg-slate-100">
                <div class="h-full rounded bg-orange-500" :style="{ width: `${row.percentage || 0}%` }" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-4">
          ...
          <div v-if="showSheet" class="space-y-2">
            <div
              ...
              :class="item.is_blank ? 'border-slate-200 bg-slate-50' : item.is_correct ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50'"
            >
              <div class="mb-1 flex justify-between text-xs text-slate-500">
                <span>سوال {{ fa(item.number) }}</span>
                <span>{{ item.is_blank ? 'بدون پاسخ' : item.is_correct ? 'صحیح' : 'غلط' }}</span>
              </div>
              <div class="mb-2 leading-6" v-html="item.question_text" />
              <p class="text-xs">
                پاسخ شما: <b>{{ item.user_answer ? String(item.user_answer).toUpperCase() : '—' }}</b>
...
const blankCount = computed(() => {
  ...
  return Math.max(0, total - (attempt.value?.total_correct || 0) - (attempt.value?.total_wrong || 0));
});
