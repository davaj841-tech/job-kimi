  ...
  <AdminLayout>
    <div class="space-y-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" class="btn-muted" @click="clearResolved">پاک‌سازی حل‌شده‌ها</button>
      </div>

      <div class="rounded-xl bg-white p-4 shadow-sm">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          <input v-model="filters.search" class="field md:col-span-2" placeholder="جستجو در پیام / کلاس / URL" @keyup.enter="load(1)" />
        ...
        <div class="mt-3 flex gap-2">
          <button type="button" class="btn-orange" @click="load(1)">اعمال فیلتر</button>
          <button type="button" class="btn-muted" @click="clear">پاک کردن</button>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              ...
              <p v-if="row.message_fa && row.message" class="mt-1 text-xs text-slate-500" dir="ltr">
                {{ row.message }}
              </p>
              <div class="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                <span class="rounded-md bg-slate-100 px-2 py-0.5 font-mono" dir="ltr">
                  {{ row.exception_class || row.class || '—' }}
                </span>
                <span>تکرار: {{ fa(row.occurrences) }}</span>
                <span>آخرین: {{ formatDateTime(row.last_seen_at || row.last_seen) }}</span>
                <span v-if="row.resolved_at" class="font-bold text-emerald-600">حل‌شده</span>
              </div>
            </div>
        ...
        <p v-if="!rows.length" class="rounded-xl bg-white py-8 text-center text-slate-500 shadow-sm">
          خطایی یافت نشد
        </p>
      </div>
