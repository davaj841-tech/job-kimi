  ...
  <div class="rounded-xl border border-surface-line bg-white p-3">
    ...
    <div v-if="!applied" class="flex gap-2">
      <input
        ...
        @keyup.enter="apply"
      />
      <button type="button" class="btn-ghost shrink-0 border border-surface-line px-4" :disabled="loading || !code.trim()" @click="apply">
        اعمال
      </button>
    </div>
    <div v-else class="space-y-1 text-sm">
      <p class="font-medium text-green-700">کد {{ applied.code }} اعمال شد</p>
      <p class="text-ink-muted">تخفیف: {{ formatPrice(applied.discount_amount) }}</p>
      <p class="font-bold">مبلغ نهایی: {{ formatPrice(applied.final_amount) }}</p>
      <button type="button" class="text-xs text-brand underline" @click="clear">حذف کد تخفیف</button>
    </div>
...
async function apply() {
  ...
  try {
    ...
    error.value = e.response?.data?.message || 'کد تخفیف نامعتبر یا منقضی شده است';
    applied.value = null;
